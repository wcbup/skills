...
description: Single-threaded concept learning. Use when the user explicitly asks to understand a topic one concept at a time, or asks to slow down an overwhelming conceptual explanation.
---

# Single-threaded Learning

Build understanding without multiplying the learner's unresolved questions. Keep one active learning thread, use a one-loop budget, and return from side questions without losing the suspended route.
...
Give one self-contained sentence that locates the topic as a whole, then teach the first useful understanding. Keep the larger route internal and choose the next step by minimum likely confusion rather than by a fixed top-down or bottom-up order.
...
This step is complete when the learner's goal and the first question to resolve are clear.
...
Before sending, predict the question the explanation is most likely to create. Aim to create no agent-created open loops. When one is necessary, create at most one. If more than one would remain, narrow the turn, explain the earliest dependency first, or replace technical labels with sufficient plain language. A core understanding may span multiple turns.

End the turn after establishing one core understanding whose explanation does not depend on an undefined term, unclear role, or missing causal step. At the main thread, ask:

> What questions do you have here, or shall we continue?

A request to continue settles the current understanding and advances the thread.
...
Stay with the active side thread until the learner signals understanding or asks to return. At its checkpoint, ask:
...
Default pace is one core understanding per turn. Explicit requests such as “go faster” widen subsequent turns while preserving one active thread and the open-loop budget. The faster pace persists through side threads and returns. A new topic resets to the default pace.

Requests such as “slow down” or “one thing at a time” restore the default pace immediately. Requests to continue preserve the current pace.

## 5. Finish the thread

Finish when the learner says the explanation is sufficient or confirms that the goal is resolved. Recompose only the main-thread conclusion in one short summary. Bring in a side-thread result only when the conclusion depends on it.
